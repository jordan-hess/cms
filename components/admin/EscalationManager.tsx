  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <p className="text-sm text-gray-500">{escalations.length} total escalations</p>
      ...
      {escalations.length === 0 ? (
        <div className="bg-white rounded-xl border border-gray-100 py-16 text-center">
          ...
          <p className="text-gray-500 font-medium">No escalations yet</p>
        </div>
      ) : (
            ...
            <div key={esc.id} className="px-5 py-4">
              <div className="flex items-start justify-between gap-3">
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    <p className="font-medium text-gray-900">{esc.customers?.name}</p>
                    <p className="text-xs text-gray-500">{esc.customers?.phone}</p>
                    <p className="text-xs text-gray-600 mt-0.5">Assigned to: <span className="font-medium">{esc.profiles?.full_name}</span></p>
                    <p className="text-sm text-gray-700 mt-1">{esc.query_description}</p>
                    {esc.possible_solution && (
                      <p className="text-xs text-gray-500 mt-0.5">Solution: {esc.possible_solution}</p>
                    )}
                    <div className="flex items-center gap-3 mt-1.5 flex-wrap">
                      ...
                      <span className="text-xs text-gray-400">{format(new Date(esc.created_at), 'dd MMM yyyy HH:mm')}</span>
                    </div>
                  </div>
                </div>
        ...
        <form onSubmit={handleSend} className="space-y-4">
          ...
          <div>
            ...
            <textarea required rows={3} value={form.query_description} onChange={e => setForm({ ...form, query_description: e.target.value })}
              ...
              placeholder="Describe the escalated issue..." />
          </div>
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">Suggested Solution</label>
            <textarea rows={2} value={form.possible_solution} onChange={e => setForm({ ...form, possible_solution: e.target.value })}
              className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-blue-500 focus:border-transparent outline-none resize-none"
              placeholder="Any guidance for the agent..." />
          </div>
